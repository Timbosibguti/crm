...
// Функция для получения данных от Rain API
async function fetchRainLeads() {
  try {
    ...
    console.log('Rain API response:', response.data);

    // Обработка данных и обновление статуса
    await updateLeadStatuses(response.data);
  } catch (error) {
    console.error('Error fetching data from Rain API:', error.message);
  }
}
...
async function updateLeadStatuses(leads) {
  try {
    for (const lead of leads) {
      console.log(`Updating lead ${lead.leadUuid}: status=${lead.saleStatus}`); // Логирование значений
